...
public interface ISuppliesRepository extends JpaRepository<Supplies, Long> {
    @Query(value="select from supplies where `name` like :name and code like :code and supplies_type_id like :supplies_type_id and supplies_type_id like :supplies_type_id and supplies_type_id like :supplies_type_id and supplies_type_id like :supplies_type_id and supplies_type_id like :supplies_type_id ",nativeQuery=true)
