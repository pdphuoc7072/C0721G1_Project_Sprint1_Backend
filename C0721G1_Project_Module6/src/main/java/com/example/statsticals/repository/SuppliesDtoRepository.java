...
public interface SuppliesDtoRepository extends JpaRepository<Warehouse, Long> {
     ...
     @Query(nativeQuery = true, value ="select s.`code`, s.`name`, w.import_quantity, ((w.import_quantity) - (w.`quantity`)) as  quantity, w.normal_supplies,((w.broken_supplies) + (w.cancelled_supplies)) as another\n" +
    ...
    @Query(nativeQuery = true, value ="select s.`name`, s.`code`, w.import_quantity, ((w.import_quantity) - (w.`quantity`)) as  quantity, o.order_date, w.normal_supplies, ((w.broken_supplies) + (w.cancelled_supplies)) as another\n" +
            "from warehouse w join supplies s on s.id = w.supplies_id\n" +
            "                 join order_detail o on s.id = o.id\n" +
